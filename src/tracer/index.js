 ...
 */
const trace = (canvas: HTMLCanvasElement, shapes: Set<Shape>) => {
  ...
  for (let i = 0; i < canvasWidth; i++) {
    for (let j = 0; j < canvasHeight; j++) {
      ...
      const [shape, point] = findClosest(shapes, ray);

      if (shape && point) {
        const normal = shape.normalVector(point);
        const light = light1;
        const lightDirection = light.subtract(point).normalize();
        const color = inShadow(point, lightDirection, withoutShape(shapes, shape)) ?
          BLACK :
          shape.material.getColor(point, normal, lightDirection, origin);
